...
Seguir Fase 4:
1. **Biblioteca — enganches restantes:** foto por etapa subida desde Producción (usar `orden_id`/`etapa`); envío del archivo por WhatsApp desde la conversación (asistido); aprobación de render que mueva la orden a `aprobacion_cliente`. (La galería base y la subida ya están.)
2. **Producción — completar:** asignar responsable desde la UI; atasco → tarea/notificación; checklist QC por tipo de pieza.
3. **Documentos — pulir:** disparo automático de contrato al confirmar; archivar firmado en la ficha; trazo de firma en canvas.
...
- **Migraciones en prod: 0004–0018 aplicadas** + `CRON_SECRET`. **Pendientes de aplicar (en orden): `0019_produccion.sql`, `0020_documentos.sql`, `0021_media.sql`.** Sin 0019 el kanban de Producción no persiste; sin 0020 la e-firma no tiene tabla `documento`; sin 0021 la Biblioteca no persiste. **Además, para la Biblioteca:** correr `supabase/storage/media_setup.sql` (crea el bucket privado `media` + policies). Las 3 migraciones están validadas en Postgres local (cadena 0001–0021 aplica limpia).
